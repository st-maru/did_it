...
RSpec.describe 'dekita!削除', type: :system do
  before do
    @user = FactoryBot.create(:user)
    @task1 = FactoryBot.create(:task)
    @task2 = FactoryBot.create(:task)
  ...
  context 'dekita!削除ができるとき' do
    it 'ログインしたユーザーは自らが投稿したdekita!の削除ができる' do
      # ログインする
      visit new_user_session_path
      fill_in 'メールアドレス', with: @task1.user.email
      fill_in 'パスワード', with: @task1.user.password
      ...
      # dekita!1に「削除」ボタンがあることを確認する
      expect(page).to have_link href: task_completion_path(@task1.id ,@task1.completions[0].id)
      # 投稿を削除するとレコードの数が1減ることを確認する
      expect{
        find_link(href: task_completion_path(@task1.id ,@task1.completions[0].id)).click
      }.to change { Completion.count }.by(-1)
      # タスクページに遷移する
      expect(current_path).to eq(task_completions_path(@task1))
      # タスクページにはdekita!1の内容が存在しないことを確認する
      expect(page).to have_no_content(@completion_summary)
      expect(page).to have_no_content(@completion_name)
      expect(page).to have_no_content(@completion_human)
      expect(page).to have_no_content(@completion_memo)
      expect(page).to have_no_content(@completion_working_day)
      expect(page).to have_no_content(@completion_start_time.hour)
      expect(page).to have_no_content(@completion_start_time.min)
      expect(page).to have_no_content(@completion_ending_time.hour)
      expect(page).to have_no_content(@completion_ending_time.min)
    end
  end
  context 'dekita!削除ができないとき' do
    it 'ログインしたユーザーは自分以外が投稿したdekita!の削除ができない' do
      # dekita!1を投稿したユーザーでログインする
      visit new_user_session_path
      fill_in 'メールアドレス', with: @task1.user.email
      fill_in 'パスワード', with: @task1.user.password
      ...
      # タスクページに移動する
      click_on @task1.name
      # dekita!2に「削除」ボタンがないことを確認する
      expect(page).to have_no_link href: task_completion_path(@task1.id ,@task1.completions[0].id)
    end
    it 'ログインしていないとdekita!の削除ができない' do
      # トップページにいる
      visit root_path
      # タスクページへ移動する
      visit task_completions_path(@task1)
